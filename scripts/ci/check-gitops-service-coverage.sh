...
#    - infra/cd/overlays/staging/<service>/kustomization.yaml
#    - infra/cd/overlays/production/<service>/kustomization.yaml
...
STAGING_DIR="$ROOT_DIR/infra/cd/overlays/staging"
PROD_DIR="$ROOT_DIR/infra/cd/overlays/production"
...
while IFS= read -r service; do
  ...
  if [[ "$managed" == "true" ]]; then
    base="no"; argo="no"; staging="no"; production="no"
    [[ -d "$BASE_DIR/$service" ]] && base="yes"
    [[ -f "$APPS_DIR/$service.yaml" ]] && argo="yes"
    [[ -f "$STAGING_DIR/$service/kustomization.yaml" ]] && staging="yes"
    [[ -f "$PROD_DIR/$service/kustomization.yaml" ]] && production="yes"

    if [[ "$base" == "no" || "$argo" == "no" || "$staging" == "no" || "$production" == "no" ]]; then
      missing=1
    fi
  fi
