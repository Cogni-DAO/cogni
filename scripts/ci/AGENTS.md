...
- `reconcile-target-substrate.sh` is the candidate app-flight target-local substrate reconciler for type:node targets. It may converge namespace, AppSet, ExternalSecret, edge/Caddy route, Postgres/Doltgres DBs, and DB inventory for one target; it must not call `deploy-infra.sh`, write OpenBao values, create legacy `<target>-node-app-secrets`, or alter existing pod-consumed DB role passwords.
