...
- `promote-build-payload.sh` translates a resolved image payload into overlay mutations via `promote-k8s-image.sh`, writes `promoted_apps=<csv>` to `$GITHUB_OUTPUT` incrementally after each successful promotion (trap EXIT guarantees the exit-time write even on abort — bug.0327), and merges per-app `source_sha` into `.promote-state/source-sha-by-app.json`. Map-write failures are `::warning::` annotations; total map-write failure exits non-zero so provenance decay cannot silently persist.
