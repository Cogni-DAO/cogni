...
REPO_URL="https://github.com/cogni-dao/cogni.git"
...
# Argo app; everything else (type: infra) is VM/compose tier.
deployable_nodes() {
  local f node
  for f in "$CATALOG_DIR"/*.yaml; do
    grep -qE '^candidate_a_branch:' "$f" || continue
    node="$(grep -E '^name:' "$f" | head -1 | sed -E 's/^name:[[:space:]]*//')"
    printf '%s\n' "$node"
  done | LC_ALL=C sort
}

# Emit one ApplicationSet object for (env, node). The git generator reads the
# node's catalog row from its own per-(env,node) deploy branch; the template
# mirrors the retired shared appset's per-node element verbatim.
render_one() {
  local env="$1" node="$2"
  cat <<YAML
# SPDX-License-Identifier: LicenseRef-PolyForm-Shield-1.0.0
...
spec:
  ...
  template:
    ...
    spec:
      ...
      source:
        repoURL: ${REPO_URL}
        targetRevision: "deploy/${env}-{{.name}}"
        path: "infra/k8s/overlays/${env}/{{.name}}"
      destination:
