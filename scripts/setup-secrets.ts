...
function setupSopsAge(env: string): void {
  ...
  // Write TF_VAR to sourceable env file
  const privKey = execSync(
    ...
    .trim();
  if (privKey) {
    const envFile = `${keyDir}/tofu-${env}.env`;
    const fs = require("fs");
    fs.writeFileSync(
      envFile,
      `# Auto-generated by pnpm setup:secrets — source before tofu apply\n` +
        `export TF_VAR_sops_age_private_key="${privKey}"\n`,
      { mode: 0o600 }
    );
    console.log(`     ${GREEN}Wrote ${envFile}${RESET}`);
    console.log(`     Run: ${CYAN}source ${envFile}${RESET} before tofu apply`);
  }
}
