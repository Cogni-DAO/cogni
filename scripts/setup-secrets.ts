...
async function main() {
  ...
  for (const env of targetEnvs) {
    ...
    // New values override existing, existing values preserved if not re-set
    ...
    console.log(
      `  ${GREEN}Saved${RESET} .env.${env} (${Object.keys(merged).length} secrets, ${Object.keys(secrets).length} new/updated)`
    );
  }
