...
const TOP_UP_AMOUNT_USD = 1; // Try $1 first; bump to $5 if API rejects
...
async function main() {
  ...
  // Step 5: Determine which function to call
  // OpenRouter's docs show swapAndTransferUniswapV3Native (ETH input).
  // If recipient_currency is USDC and they support token input, we'd use transferTokenPreApproved.
  // For now, follow the docs: use swapAndTransferUniswapV3Native with ETH.
  //
  // The total ETH value = recipientAmount + feeAmount + slippage buffer.
  // OpenRouter says "include a little extra — excess refunded."

  const totalRequired = intent.recipientAmount + intent.feeAmount;
  // Add 5% buffer for swap slippage (excess is refunded)
  const ethValue = (totalRequired * 105n) / 100n;

  console.log(`\n[exp1] Using swapAndTransferUniswapV3Native (ETH input)`);
  console.log(`  recipientAmount + feeAmount = ${totalRequired} wei`);
  console.log(`  ETH value (with 5% buffer)  = ${ethValue} wei`);

  // Step 6: Simulate
  console.log(`\n[exp1] Simulating transaction...`);
  try {
    await publicClient.simulateContract({
      address: contractAddress,
      abi: TRANSFERS_ABI,
      functionName: "swapAndTransferUniswapV3Native",
      args: [intent, 3000], // 3000 = 0.3% Uniswap fee tier (standard)
    ...
    console.error(
      `\n[exp1] This might mean we need a different function or fee tier.`
    );
    console.error(
      `[exp1] Try: transferTokenPreApproved (USDC input) or different poolFeesTier.`
    );
    process.exit(1);
  }

  // Step 7: Execute
  console.log(`\n[exp1] Broadcasting transaction...`);
  ...
  console.log(`\n[exp1] ═══ SPIKE FINDINGS ═══`);
  console.log(
    `  Function used:     swapAndTransferUniswapV3Native (ETH input)`
  );
  console.log(`  Contract address:  ${contractAddress}`);
  console.log(
    `  Matches expected:  ${contractAddress.toLowerCase() === TRANSFERS_CONTRACT.toLowerCase()}`
  );
  console.log(`  Pool fee tier:     3000 (0.3%)`);
