...
adopt() {  # adopt <tofu-address> <id-or-empty> <human-label>
  ...
  [[ -z "$id" ]] && return 0
  # Laptop re-runs keep tfstate on disk, so the resource is already managed —
  # `tofu import` would hard-fail ("Resource already managed by OpenTofu").
  # Only import when the address is absent from state (the GHA ephemeral case).
...
# ControlMaster multiplexing: the per-node secret seed (Phase 5c) makes one
# SSH call per key — multi-node fan-out is 4 nodes × ~15 keys × (reconcile +
# seed) ≈ 120 connections in seconds, which trips sshd rate-limiting
# ("Connection closed by <ip> port 22") mid-seed. Reuse a single master TCP
# connection for every call so there's no per-call TCP/auth handshake.
# Short, fixed /tmp path — ControlPath Unix sockets cap at ~104 chars and
# macOS $TMPDIR is long enough to blow that with the %r@%h:%p expansion.
...
if [[ -n "$SEED_TOKEN" ]]; then
  ...
  for ref in "${BRANCHES_TO_SEED[@]}"; do
    ...
    # gh 2.83+ leaks the 404 error body (`{"message":"Branch not found",...}`)
    # to stdout even through `--jq`, so the `|| echo ""` idiom captures the body
    # instead of empty. Only a real 40-hex commit SHA means the branch exists;
    # anything else (error JSON, blank) → treat as absent → create path below.
...
if [[ -n "$FORK_ROOT" && "$FORK_ROOT" != "null" ]]; then
  ...
  # Rewrite both the per-env wrapper directory AND the shared _template (when
  # present). `_template` is optional — only pass overlay roots that exist, or
  # `find` errors on the missing path and `set -o pipefail` aborts the script.
...
# applies server-side.
# ── 5b.0 Enable Helm in Argo's kustomize build ───────────────────────────
# The openbao + external-secrets layers are kustomize-with-helm
# (HelmChartInflationGenerator). Upstream argocd install.yaml does NOT set
# `kustomize.buildOptions: --enable-helm`, so the repo-server's
# `kustomize build` hard-fails ("must specify --enable-helm") and both
# Applications stick at sync=Unknown. Patch argocd-cm + restart repo-server
# before registering the apps. Idempotent (merge patch + rollout).
log_info "  5b.0 — enabling --enable-helm in argocd-cm (kustomize-with-helm substrate)"
