...
- **candidate-a flight is split in two; preview/prod isn't** ([`bug.5015`](https://cognidao.org)). `candidate-flight.yml` ships the app digest (Argo + migrator, **never** runs `deploy-infra`); `candidate-flight-infra.yml` runs `deploy-infra.sh` (OpenFGA model bootstrap + compose + rollout restart). `promote-and-deploy.yml` does **both** in one graph for preview/production — so candidate-a is the only env where a substrate/model change (e.g. a new/renamed RBAC relation like `production_promoter`) needs a **second manual dispatch**, or it silently half-deploys: the app ships but the model doesn't, and the gated route returns `503 authz_unavailable` (not `authz_denied`) until someone remembers the infra lever. candidate-a does **not** mirror preview/prod.
