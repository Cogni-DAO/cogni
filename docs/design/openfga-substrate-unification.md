...
### 🟡 Finding 3 — BaaS lens: OpenFGA is shared substrate, not a node. Name the catalog file + path.

`node-baas-architecture.md` invariant: *node declares shape; operator wires environment.* OpenFGA is **shared substrate** (peer of postgres/litellm/redis) — provisioned by the operator plane but consumed by any node, **owned by no node**. Its DB credential is therefore a **shared-infra credential**: it goes in **`infra/secrets-catalog.yaml`** (the shared/infra catalog) at `cogni/<env>/openfga/OPENFGA_DB_PASSWORD` (dedicated service path; `_shared` only if litellm/openfga deliberately share), **never** a `nodes/<node>/.cogni/secrets-catalog.yaml` — because it is cross-cutting infra, *not* because it belongs to the operator. Make the catalog file + path explicit in Migration-A so it isn't filed as a node secret. (The BaaS map's *operator provides DB, roles, RLS, backups* row confirms the `db-backup` parity you pulled into A is correctly operator-provisioned.)
...
The `openfga` DB + login role are **one shared-infra role regardless of how many apps consume the API** (one server, one DB, created once in the infra-DB pass). The provision/deploy-split lane-ownership question is independent of consumer count. Stands as written.

### Finding 3 — CORRECT for the DB password; that's the litellm precedent, and the all-nodes future _confirms_ it.
`OPENFGA_DB_PASSWORD` → `infra/secrets-catalog.yaml` @ `cogni/<env>/openfga/*`, never a node catalog — **roadmap-stable.** The litellm analogy is exact: litellm is shared infra whose DB cred is a single shared credential *and* whose runtime config is fanned to every node. Nodes consuming OpenFGA's API no more makes its DB credential per-node than nodes calling litellm makes litellm's per-node. So Phase A is correct as designed; the all-nodes trajectory does not touch it.
...
1. **Phase A — unchanged.** One shared-infra DB role + `infra/secrets-catalog.yaml` cred. Correct; roadmap-stable.
