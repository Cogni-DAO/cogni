...
1. **Dedicated DB role on the OpenBao SSOT — net-new plumbing in the infra-DB pass.**
   ...
   - `OPENFGA_DB_PASSWORD` is a **shared-infra credential** → declared in **`infra/secrets-catalog.yaml`** at `cogni/<env>/openfga/*` (dedicated service path; `_shared` only if litellm/openfga deliberately share), **never** a `nodes/<node>/.cogni/secrets-catalog.yaml`. It's cross-cutting infra, not the operator's.
