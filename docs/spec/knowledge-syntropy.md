...
| DOMAIN_FK_ENFORCED_AT_WRITE                | Every write to `knowledge` (HTTP contribution and `core__knowledge_write`) verifies `domain` exists in `domains` before INSERT, via a shared `assertDomainRegistered(client, domain)` helper. Unregistered domains return `DomainNotRegisteredError` → HTTP 400. Closes the gap where `ENTRY_HAS_DOMAIN` was a wish, not a gate. |
| DOMAIN_REGISTRY_VIA_UI                     | New domains are registered via cookie-session UI (`POST /api/v1/knowledge/domains`), never by the migrator. Preserves `NODES_BOOT_EMPTY`: even base domains (`meta`, `prediction-market`, etc.) are seeded by an operator clicking through the `/knowledge` Domains mode on first visit.                                         |
...
| **P1** — EDO-aligned `entry_type` + `citations[]` + `evaluate_at`                    | Hypothesis rows can be written + retrieved + cited; outcomes can validate them via `validates` / `invalidates` citation edges. Closes the "confidence drifts over time" mechanic at the column level.                                                                                                                                                 | Designed. Filed when P0.5 merges.                                                                       |
