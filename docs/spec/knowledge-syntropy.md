...
| **P1** — EDO Foundation (`proj.edo-foundation`)                                      | Closes the data → hypothesis → action → data loop. Schema extension (`evaluate_at` + new entry*type + citation type values) · `EdoResolverPort` + 1-hop `recomputeConfidence` · three atomic agent tools (`core\_\_edo*{hypothesize,decide,record_outcome}`) · `resolveDueHypotheses`cron in`scheduler-worker`· brain prompt teaches the loop ·`/knowledge?mode=chains` DAG view. Designed in § The EDO Loop above; broken into 6 PR-sized tasks under one project. | **Designed.** Filed as `proj.edo-foundation` when P0.5 merges.                                          |
