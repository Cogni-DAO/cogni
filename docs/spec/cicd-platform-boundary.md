...
**⚠️ Known gap — shared-infra substrate has no continuous delivery.** The `infra/k8s/argocd/{openbao,external-secrets,reloader}-application.yaml` Applications are applied **once at provision** (`register-substrate-apps.sh` / `provision-env-vm.sh`), pinned to a deploy-branch ref, and **never re-applied by any flight/promote/deploy workflow.** Argo reconciles each App's _contents_ once it exists, but a change to the App's own manifest (e.g. `infra/k8s/argocd/openbao/values.yaml`) reaches no cluster without a manual `targetRevision`→`main` re-point + pod-roll. This stranded **#1617** (OpenBao 1Gi, 12 days) and **#1788** (the OpenBao OOM-leak cure, bug.5051), and forced the **#1762** Reloader manual backfill. The freeze routes platform work "to the substrate" — but for _shared-infra_ substrate that delivery path does not exist yet (node substrate ships fine via the Axiom 22 `materialize → reconcile` lane; app digests ship via deploy branches). Fix: an **app-of-apps / root Application** tracking `main` that owns these 3 Application CRs, so shared-infra substrate rides CD like the other two tiers already do.
...
| Change **shared-infra** substrate config (OpenBao server values, ESO/Reloader version/config) | re-point `infra/k8s/argocd/<app>-application.yaml` `targetRevision`→`main`, then (until app-of-apps lands) **manually** re-apply the Argo Application CR + pod-roll per env | assume a flight/promote/deploy workflow delivers it — **none does** |
