...
| Observability Access | which substrates it emits to (logs/analytics/DB)                                     | per-principal, per-node-scoped READ credentials issued on `developer` grant (operator is an issuer, not a query proxy) — see [Substrate Access-Grant Plane](./substrate-access-grant.md) |
