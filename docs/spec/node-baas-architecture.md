...
Every node maintains exactly one such entry; agents **refine it in place** (REFINE_OVER_EXTEND) whenever repo layout, scripts, CI, deploy, auth, or validation behavior changes — it is the living map, not a one-time doc. The bundle surfaces a bounded excerpt and points at the full entry. New nodes adopt this via `Cogni-DAO/node-template` (external repo): node-template ships the starter `<slug>-agent-orientation` entry pattern that each fork refines; per-node knowledge seeds are ported repo-by-repo (operator base seeds do not auto-propagate to forks).
