...
the substrate sharing model (one shared generic worker serves all nodes) and the
node-direct-create evolution are in [Temporal Substrate](./substrate-temporal.md).
...
| Temporal / Recurring | `repo-spec.yaml` `schedules[]` (cron + `route` **XOR** `graph` + payload) + ops **route(s)** to receive dispatch; **no** custom workflow code (a per-node worker is an opt-in sovereign escape hatch) | reconciles `schedules[]` into Temporal under the node's tenant identity (`syncNodeSchedules`, `SYSTEM_OPS_ONLY`); **one shared worker** runs only generic workflows (`NodeTaskWorkflow`/`GraphRunWorkflow`) on a per-node `scheduler-tasks-<id>` queue; namespace-scoped creds via ESO — execution model in [Temporal Patterns](./temporal-patterns.md), sharing/sovereignty + node-direct-create target in [Temporal Substrate](./substrate-temporal.md) |
...
| Observability Access | which substrates it emits to (logs/analytics/DB)                                                                                                                                                      | per-node-scoped READ on `developer` grant — operator **proxies** the query pinned to `{node="<id>"}` (dev holds no env-wide token), not a credential issuer — see [Substrate Access-Grant Plane](./substrate-access-grant.md)                                                                                                                                                                                                                             |
...
| Orientation excerpt | bounded excerpt of the `<slug>-agent-orientation` hub entry                | knowledge hub |
...
The bundle's stance is **constitution + map**: the code-owned invariants say how every agent must behave; the repo-spec mission says why this node exists; the orientation excerpt is the current-node map an agent needs to start (where to edit, what not to run, what can break prod/candidate, what to recall next). It stays **index-first** — a single bounded orientation excerpt is the only entry body inlined; everything else is pointers.
...
Every node maintains exactly one living orientation entry; agents **refine it in place** (REFINE_OVER_EXTEND) whenever repo layout, scripts, CI, deploy, auth, or validation behavior changes — it is the living map, not a one-time doc. The bundle surfaces a bounded excerpt and points at the full entry, preferring the node-specific `<name>-agent-orientation` over the generic starter.
