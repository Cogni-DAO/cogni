---
id: spec.cd-pipeline-e2e-legacy-canary
type: spec
title: CD Pipeline E2E — Legacy Canary GitOps
status: draft
trust: draft
summary: Historical canary/staging-era specification for continuous deployment of operator + node apps via Argo CD on k3s
...
# CD Pipeline E2E: Legacy Canary GitOps
...
> Legacy note: this document still contains historical canary/staging-era workflow and branch assumptions. For the current target branch model, pre-merge candidate-slot flow, and post-merge promotion rules, treat [`docs/spec/ci-cd.md`](./ci-cd.md) as the source of truth.
