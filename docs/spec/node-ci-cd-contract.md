...
| --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `node`    | `scripts/ci/reconcile-target-substrate.sh` first converges target-local substrate, then `scripts/ci/assert-target-substrate.sh` checks VM/k3s/Argo reachability, namespace, per-target AppSet/Application, local catalog/overlay/AppSet, scoped DNS, edge/Caddy route, Service NodePort, ESO-only Deployment secret consumption, Postgres DB, and Doltgres DB. | Reconcile target-local rows, fail loud on unowned substrate, then prove read-only; do not run `deploy-infra.sh`.                                           |
...
5. **Substrate is reconciled and asserted before app rollout.** For target shapes enabled in app flight today, `candidate-flight.yml` reconciles target-local AppSet/DNS/edge/ESO/Postgres/Doltgres/DB-inventory rows, then runs the read-only target substrate gate. Missing unowned substrate (VM, base Compose services, OpenBao/ESO, `<env>-db-reader`, OpenBao values, source artifact) stops the app flight before digest promotion.
...
| `scripts/ci/reconcile-target-substrate.sh` | Narrow per-target substrate reconciler used before selected app flights                        |
