...
3. **Standalone `node-template` template repo (the real prerequisite).** GitHub generate-from-template (`POST /repos/{tpl}/generate`) copies all files server-side — but `node-template` is only a monorepo _subdirectory_ (`nodes/node-template/`), not a standalone repo. Minting a node repo without it means ~1100 per-file `createBlob` calls (the exact wall #1462's inline path was built to avoid). So a standalone `Cogni-DAO/node-template` template repo must exist + stay in lockstep with `nodes/node-template/` (a sync question). This gates the repo-mint step; `openNodeSubmodulePr` injects its result (`nodeRepoUrl` + `nodeRepoHeadSha`) so the operator-side composition is buildable + testable independently.
