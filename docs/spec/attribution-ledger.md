...
| SOURCE_NO_ADAPTER                | `collectFromSource` and `resolveStreams` throw if no poll adapter is registered for a configured source **whose `ingest` mode is `poll` or `both`** (the default). At bootstrap, `container.ts` cross-checks repo-spec `activity_sources` against registered adapters and logs `CONFIG_SOURCE_NO_ADAPTER` at error level for coverage gaps. Prevents silent zero-activity ingestion. Does NOT apply to `ingest: webhook` sources (see WEBHOOK_ONLY_SOURCE).                                                              |
| WEBHOOK_ONLY_SOURCE              | A source may declare `ingest: webhook` in repo-spec `activity_sources.<source>`. `CollectSourcesWorkflow` then SKIPS that source — no `resolveStreams`/`collectFromSource`, no poll adapter required, no `SOURCE_NO_ADAPTER`. Receipts arrive solely via the webhook receiver (WEBHOOK_RECEIPT_APPEND_EXEMPT) and are still selected into the epoch. Use when the poll plane has no credentials — e.g. **git**, where the operator GitHub App webhook is the live path and the scheduler-worker holds no GH App key by design. Tradeoff: `POLL_RECONCILES_WEBHOOKS` does not hold for webhook-only sources until an operator-hosted reconcile poll lands (vNext); webhook misses are not yet backfilled. Omitting `ingest` (or `poll`/`both`) preserves the fail-loud poll contract.                                            |
...
- **V0 (operator-only, current target).** Only the operator's own repo (`cogni-dao/standalone-node`) is configured, `ingest: webhook` (WEBHOOK_ONLY_SOURCE). Operator's webhook receipts are already deposited; the only fix needed is to stop `CollectEpoch` from crashing on the absent poll adapter. This proves the entire ingest → select → enrich → allocate → finalize → statement loop for one node, one source.
- **vNext (node-template / multi-node).** Three additive seams, none of which touch the per-node ledger contract:
  ...
  3. **Operator-hosted reconcile poll** (restores `POLL_RECONCILES_WEBHOOKS` for git without violating the no-GH-key-on-worker rule): the operator mints a short-lived per-repo token from its GitHub App and runs the poll itself, flipping git's `ingest` to `both`. The scheduler-worker never holds the App key.
