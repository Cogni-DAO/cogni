...
#### When you need a per-node worker (the escape hatch)

If your recurring work needs **custom durable workflows** — signals, child workflows, long
human waits, multi-step orchestration that cannot be expressed as "fire my route" — graduate
to your **own** per-node worker: it registers your custom workflows + activities and polls
**your** per-node queue; execution is fully in-node, operator out of it. This is **opt-in**
(it costs a worker pod per node) and is **never the default** — see
[substrate-temporal.md](./substrate-temporal.md) § the sovereign escape hatch.
