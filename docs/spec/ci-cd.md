...
**Reader**: `scripts/ci/verify-buildsha.sh` in `SOURCE_SHA_MAP` mode. For each Ingress-probeable app in the map, curls `/version`, asserts `.buildSha == map[app]`. Fails red on any mismatch. (The probe is the dedicated `/version` endpoint, not `/readyz`, so that a legitimate infra-degraded 503 on `/readyz` cannot false-fail an artifact-identity check — task.0345 / PR #978.)
...
- **task.0349 — CI-owned seed (preferred path)** — After **Flight Preview** completes successfully for a merge to `main`, [`promote-preview-digest-seed.yml`](../../.github/workflows/promote-preview-digest-seed.yml) runs [`scripts/ci/promote-preview-seed-main.sh`](../../scripts/ci/promote-preview-seed-main.sh) (**Option B**: direct `promote-k8s-image.sh --no-commit` calls; does **not** reuse `promote-build-payload.sh`, which is coupled to deploy-branch `.promote-state/` provenance). **Tri-state per image:** resolve `preview-{mergeSha}{suffix}` when the merge produced/retagged that image; else **retain** the existing pin from the overlay if it still resolves in GHCR; else **fail**. Emits at most **one** `chore(preview): …` commit to `main`. **Timing:** this runs _after_ Flight Preview finishes; `flight-preview.sh` may already have dispatched `promote-and-deploy` for the same merge — the seed commit primarily protects **future** unrelated preview promotes, not the in-flight deploy for that SHA.
- **bug.0344 — Argo CD Image Updater (legacy / transitional)** — May remain installed and annotated until demoted in follow-up work. When enabled, it polls `preview-*` tags and writes `main` preview overlays under prefix `chore(deps): argocd-image-updater`. Bootstrap: [docs/runbooks/image-updater-bootstrap.md](../runbooks/image-updater-bootstrap.md). **Scope guard:** [`scripts/ci/check-image-updater-scope.sh`](../../scripts/ci/check-image-updater-scope.sh) allowlists which AppSets may carry `argocd-image-updater.argoproj.io/*` annotations. Candidate-a and production stay excluded from IU auto-writes (candidate-flight overwrites `deploy/candidate-a`; production is human-gated via `promote-to-production.yml`).
- **Prettier + machine YAML** — `infra/k8s/overlays/**/kustomization.yaml` may remain prettier-ignored while Image Updater's Go YAML writer is still in play; revisit when IU annotations are removed (task.0349 PR B).
...
- [ ] **Candidate-a + production seed freshness beyond bug.0344 MVP**
      bug.0344 ships Argo CD Image Updater covering `main`'s `preview/` overlays only, for both app and migrator images via a split GHCR package (`cogni-template` + `cogni-template-migrate` — one image updater alias per image, not one per tag class). Candidate-a was descoped post-#974 merge: the per-merge write rate (~3 commits to main per merge-to-main, in perpetuity, per Application) wasn't worth the marginal seed-freshness value when candidate-flight already overwrites `deploy/candidate-a` digests on every slot acquisition. Production overlays on main remain intentionally human-gated via `promote-to-production.yml`. Enforcement is handled — `scripts/ci/check-image-updater-scope.sh` fails closed on any new AppSet outside the allowlist. Still open: a detection signal for candidate-a + production seed staleness (Loki/Grafana query on overlay-digest age), an empirical confirmation that `update-strategy: latest` orders by merge-SHA and not by manifest-creation-time in out-of-order PR-build scenarios, and a design for write-rate-minimized candidate-a auto-updates if that path is ever revisited (e.g. only write when the seed is ≥N days stale).
