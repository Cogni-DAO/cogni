...
21. **`TARGET_SUBSTRATE_IS_RECONCILED_PER_ENV`** (node-formation parity). Per-node public DNS and target-local substrate are **env-scoped** promotion-lane concerns. `candidate-flight.yml` keeps the app lever digest-only, but before app promotion it runs the narrow target substrate path for nodeRef / node-birth flights: `reconcile-node-dns.sh <env>` upserts the node A record, `reconcile-appset` applies only the target AppSet, and `reconcile-target-substrate.sh` converges the target's namespace, ExternalSecret, edge/Caddy route, Postgres DB, Doltgres DB, and `COGNI_NODE_DBS` membership. The read-only `assert-target-substrate.sh` gate then proves the result before any deploy branch digest write. Missing broad env substrate remains fail-loud: no VM, no base Compose services, no OpenBao/ESO, no `<env>-db-reader`, or missing OpenBao values stops the app flight. Do not run `deploy-infra.sh` from app flight.
...
**NodeRef / node-birth substrate path.** Independence still holds: app flight does not run the broad Compose infra deploy. For node-shaped targets that need target-local substrate, `candidate-flight.yml` runs the narrow reconciler before the read-only assertion. That reconciler may apply the target AppSet and ExternalSecret, update the target edge route, create missing target DBs from OpenBao-sourced role credentials, and update DB inventory. It must not rebuild source artifacts, write OpenBao values, recreate `<target>-node-app-secrets`, or alter pod-consumed DB role passwords.
