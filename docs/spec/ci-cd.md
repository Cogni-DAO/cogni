...
- **main's overlay digests are eventually-consistent with GHCR** (bug.0344). Argo CD Image Updater (`infra/k8s/argocd/image-updater/`, v0.15.2) polls the `preview-*` tag class every 2 minutes and commits fresh digests back to `main:infra/k8s/overlays/{preview,candidate-a}/<app>/kustomization.yaml` — both app and migrator images — authored by `github-actions[bot]` under commit-prefix `chore(deps): argocd-image-updater`. Authentication is `ACTIONS_AUTOMATION_BOT_PAT` (`Cogni-1729`) via the admin + `enforce_admins: false` carve-out on main that existing PR merges already rely on. Every unrelated flight's rsync (`main → deploy/<env>`) then inherits a current seed. Bootstrap + rollback: [docs/runbooks/image-updater-bootstrap.md](../runbooks/image-updater-bootstrap.md). **Do not extend `promote-k8s-image.sh` or add bespoke GitHub Actions to close the digest-seed freshness gap — that is reinvention of the controller.** MVP covers `preview/` + `candidate-a/` overlays on main, app + migrator images for all node types (`operator`, `poly`, `resy`). Production-overlay seed freshness, detection signals, and enforced write-scope rollback are the three open design surfaces tracked in bug.0344 § _Implementation review (revision 3)_ B12 — landing inside this PR, not a follow-up. bug.0344 is the single source of truth for scope; do not restate it here.
...
- [ ] **Production-overlay seed freshness beyond bug.0344 MVP**
      bug.0344 ships Argo CD Image Updater covering `main`'s `preview/` + `candidate-a/` overlays for both app and migrator images via a split GHCR package (`cogni-template` + `cogni-template-migrate` — one ACIU alias per image, not one per tag class). Production overlays on `main` are intentionally out-of-scope for auto-commit — `promote-to-production.yml` remains the human-gated path. Open: a detection signal for production seed staleness vs. preview (Loki/Grafana query on overlay-digest age), a CI enforcement check that fails the build if anyone adds ACIU annotations to `production-applicationset.yaml`, and an empirical confirmation that `update-strategy: latest` orders by merge-SHA and not by manifest-creation-time in out-of-order PR-build scenarios. Tracked as bug.0344 § B12 (a/b/c) + C2 + C3.
