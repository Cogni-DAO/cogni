...
| `Cogni-DAO/node-template`   | Canonical node-at-root template repo — Publish **seeds a standalone `Cogni-DAO/<slug>` from its history** (shared merge base, _not_ a GitHub fork-network member), commits node identity on top, then submodule-pins it at `nodes/<slug>`. The operator repo does not carry a duplicate source tree. |
...
1. **Mint** — create `Cogni-DAO/<slug>` as a **standalone repo seeded from `node-template`'s history** (NOT a GitHub fork): create an empty repo, then push `node-template@main`'s full commit history into it. The two repos share a **git merge base** but `<slug>` is **not a member of node-template's GitHub fork network**. Shared _history_ — not GitHub's fork _relationship_ — is what lets node developers fetch and merge upstream template updates (`SHARED_HISTORY_NOT_FORK_FLAG`). _Prior model: a `default_branch_only` fork via `POST /repos/.../forks`; abandoned because the fork network is one-fork-per-account-per-network (a multi-node contributor like `flock-leader` cannot fork each node independently), forces the inherited `is_template`/`forked-from` lineage onto every sovereign node, and conflates object-sharing with ancestry — see `UPSTREAM_SYNC_PUSHES_OBJECTS` below._
...
- `SOVEREIGN_NODE_REPO` — a minted node is a **sovereign repo**: its own GitHub fork network (independently forkable by any contributor with zero collisions), no inherited `is_template` flag, no "forked from node-template" lineage. It shares node-template's git **history**, never its fork-network membership. Aligns with the network's own promise — "strongest within the network, forkable to run fully sovereign."
- `UPSTREAM_SYNC_PUSHES_OBJECTS` — the load-bearing consequence of `SOVEREIGN_NODE_REPO`. A standalone node repo does **not** share node-template's git object store (no fork network), so the operator upstream-sync (`syncTemplateUpstreamToFork`, [repo-sync-contract.md](repo-sync-contract.md)) must **fetch node-template's objects and push them into the node repo** before opening the merge PR — it can no longer materialize a node-template SHA via a same-network ref (`github-repo-write.ts` `upsertRef(UPSTREAM_BRANCH, templateSha)`, the prior fork-network shortcut). The 3-tier carve-out / `FORK_FREEDOM` / `TIER3_NEVER_SYNCED` semantics are unchanged — only the object-availability step is added.
- `ANCESTRY_IS_MERGE_BASE_NOT_FORK_FLAG` — repo-reuse / lineage validation checks that a same-named existing repo **shares node-template's merge base** (`git merge-base` resolves), not that GitHub reports `fork == true`. (Supersedes the node-wizard "Repo Ancestry Rule" fork-flag check.)
