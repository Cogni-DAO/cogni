...
`pr-build.yml` is `detect → build (matrix, one leg per affected target) → manifest` as of task.0321 (PR #896, 2026-04-17). Candidate-a has two orthogonal levers (task.0314, PR #883, merged 2026-04-16): `candidate-flight.yml` (app digests → Argo) and `candidate-flight-infra.yml` (VM compose). Preview/prod (`promote-and-deploy.yml`) runs both sequentially on every merge. Per-node flighting lands in two parts: **task.0320** ships the substrate across all three envs (per-env per-node `deploy/<env>-<node>` branches + 4 git generators per AppSet — Kargo Stage primitive on existing infrastructure, behavioral no-op at merge); **task.0372** ships the matrix cutover of the three flight workflows (`strategy.matrix` + `fail-fast: false` + Turbo-affected fan-out + per-node concurrency groups). Post-cutover, a failed verify on one node cannot fail another node's lane — the resy-stuck-hook class failure (candidate-flight run 24910378351, 2026-04-24) becomes structurally impossible on all three environments.
