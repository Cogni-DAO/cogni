...
| OpenTofu: Provision k3s cluster (single node MVP)                              | In Progress | 3   | task.0149 |
| Install Argo CD on k3s                                                         | In Progress | ↑   | task.0149 |
| Migrate scheduler-worker from Compose to k3s                                   | In Progress | ↑   | task.0149 |
| Promotion flow: PR to change image digest in overlay → Argo syncs              | In Progress | ↑   | task.0149 |
| Storage plan: PVCs for stateful deps (postgres data), backup strategy          | Not Started | 2   | —         |
| ArgoCD manages apps only; infra via OpenTofu + bootstrap manifests             | In Progress | ↑   | task.0149 |
| Retire SSH deploy for services (keep for app until P2)                         | In Progress | ↑   | task.0149 |
