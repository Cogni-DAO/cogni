...
Candidate-a has two orthogonal levers (task.0314, PR #883, merged 2026-04-16): `candidate-flight.yml` (app digests → Argo) and `candidate-flight-infra.yml` (VM compose). Preview/prod (`promote-and-deploy.yml`) runs both sequentially on every merge.
...
| 20  | **scheduler-worker CrashLoop + sandbox-openclaw ImagePullBackOff block `wait-for-argocd`**                                                                                                                                                                                                 | ❌ RED       | —     | SW: manually-applied k8s secret shadows configmap `COGNI_NODE_ENDPOINTS` with UUID-only keys (no `operator=`). Fix: patch secret to remove shadowing keys. Openclaw: placeholder image tag never promoted. Fix: `wait-for-argocd` scoped to promoted apps only.                                                                                                                                                                                               |
