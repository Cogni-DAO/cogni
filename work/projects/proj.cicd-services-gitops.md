...
| 8   | **No production promotion in pipeline** — promote-and-deploy supports it but nothing triggers it                                                                                                                                                                                           | 🟡 IN PR     | —     | `promote-to-production.yml` (manual dispatch) + `scripts/ci/promote-to-production.sh`: reads preview overlay digests, syncs base/catalog from `main`, opens PR `promote-prod/* → deploy/production`. Merge pushes `deploy/production`, which triggers `promote-and-deploy.yml` via `on: push` (resolves `source_sha` from `.promote-state/source-sha`). Argo CD reconciles prod pods. First live prod flight still pending.                                                                |
...
| 21  | **🔴 P0: Hand-curated overlay digests — adopt a digest-update controller (bug.0344)**                                                                                                                                                                                                      | ❌ RED (P0)  | —     | Main's `infra/k8s/overlays/*/<service>/kustomization.yaml` digest fields are hand-maintained seeds. Flights rsync them onto deploy branches and bump only affected apps; services not touched by a PR inherit stale seeds → silent regression. Produced #970 (migrator `ERR_PNPM_NO_IMPORTER_MANIFEST_FOUND`) and #971 (scheduler-worker pre-multi-queue → 30-40s silent chat hang on preview + candidate-a). Fix: Argo CD Image Updater (preferred — Argo-native) or Flux `ImageUpdateAutomation` / Renovate digest-pinning as equivalents. Controller watches GHCR → commits fresh digests to `main`. Retires the manual-bump class permanently. **Do NOT build a bespoke GitHub Actions workflow for this — that is reinvention of an existing tool.** |
