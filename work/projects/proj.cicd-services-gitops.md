...
4. **task.0372 — Matrix cutover (next; symmetric across all 3 envs).** Refactors 3 AppSets (1 git generator → 4 per-node generators each) and matrix-fans `candidate-flight.yml` / `flight-preview.yml` / `promote-and-deploy.yml` with `fail-fast: false`. Preview adds an `aggregate-preview` job that updates `deploy/preview/.promote-state/current-sha` after all-cells-green so `release.yml`'s read is unchanged. Production aggregator updates `deploy/production/.promote-state/current-sha` symmetrically. Per-node lane isolation across all 3 envs in one atomic PR — symmetric architecture is the discipline; per-env divergence is the tax we refuse to pay.
