...
| Mirror placement policy v0 — single resting GTC limit per (target, market), DB partial-unique-index dedupe, cancel-on-target-SELL, 20-min TTL sweeper                    | In Review    | 5   | task.5001 ([API](https://preview.cognidao.org/api/v1/work/items/task.5001))   |
...
| [bug.0428](../items/bug.0428.poly-redeem-worker-hardcodes-usdce.md) — redeem-worker hardcodes USDC.e collateralToken                       | needs_triage    | V2 vanilla CTF positions silently bleed at redeem (collateralToken mismatch). Dormant — bites once a V2 vanilla CTF position resolves. NegRisk redeems unaffected.                                                                                                                                                  |
| [bug.0430](../items/bug.0430.poly-mirror-cap-leaks-on-error-rows-that-fill.md) — per-position cap leaks because `error` rows fill on chain | needs_implement | **CRITICAL.** Production observed 2026-04-29: $47 pUSD drained to $0.60 in ~25min via silent on-chain fills the ledger stamped `error`. task.0424's `cumulativeIntentForMarket` excludes `error` → cap reads $0 → unbounded placement. Copy trading currently UNSAFE TO RUN.                                        |
| [bug.0431](../items/bug.0431.poly-redeem-policy-misclassifies-winners-as-losers.md) — redeem policy stamps winners as losers               | needs_implement | $14.99 of confirmed-winning vanilla CTF positions on prod sat `lifecycle=loser status=skipped attempt_count=0` until force-flipped. Policy mis-judges binary-market outcomes for a subset of conditions. 20 other conditions redeemed correctly, so the worker path works — bug is in `resolve-redeem-decision.ts`. |

## Phase 5 (P5) — Wallet-cycle continuity (kill the human-click-on-cycle dependency)

> **Active.** Surfaced production 2026-04-29 ~07:30Z: prod funder held $46.66 USDC.e + $0.42 pUSD, and 154/154 mirror placements returned `errorCode: insufficient_balance`. V2 exchanges spend only pUSD; deposits + V1-legacy redeems arrive as USDC.e; the only existing wrap path is Step 2 of the Enable Trading ceremony, which fires once at button-click time and never again. The product cannot be left to run unattended without a closed cycle.

| Deliverable                                                                                                            | Status       | Est | Work Item                                                           |
| ---------------------------------------------------------------------------------------------------------------------- | ------------ | --- | ------------------------------------------------------------------- |
| Money page Reset Trading Approvals button — null `trading_approvals_ready_at` so users can re-fire the ceremony in-app | Needs Triage | 2   | [task.0421](../items/task.0421.money-page-trading-reset-button.md)  |
| Auto-wrap USDC.e → pUSD — explicit-consent server loop (Money page toggle + bootstrap job + observability)             | Needs Design | 5   | [task.0429](../items/task.0429.poly-auto-wrap-usdce-to-pusd.md)     |
| V2 vanilla CTF redeem to pUSD — pass `collateralToken=pUSD` for V2 positions, `=USDC.e` for V1; closes the V2 cycle    | Needs Triage | 3   | [bug.0428](../items/bug.0428.poly-redeem-worker-hardcodes-usdce.md) |
...
- **The wallet's currency cycle is one-shot per click in v0/v1.** Step 2 of Enable Trading wraps USDC.e → pUSD once. After that, deposits + V1-legacy redeems arrive as USDC.e and stall the trade path on `insufficient_balance` until a human re-clicks. P5 closes this — until P5 ships, every prod incident where USDC.e drifts back into the wallet requires manual SSH + click. Tracked as [task.0429](../items/task.0429.poly-auto-wrap-usdce-to-pusd.md).
