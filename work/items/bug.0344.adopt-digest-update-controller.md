...
- [ ] **WRITE_BACK_SCOPE_EXCLUDES_PRODUCTION_OVERLAY** — ACIU writes to `main` only, and only to `infra/k8s/overlays/{preview,candidate-a}/<app>/kustomization.yaml` — never to production overlays, never to deploy branches. Deploy-branch digest promotion (`promote-k8s-image.sh` during flight workflows) stays as-is. Verify: no `argocd-image-updater.argoproj.io/git-branch` annotation points at a `deploy/*` branch; `infra/k8s/argocd/production-applicationset.yaml` has zero ACIU annotations (enforced by `scripts/ci/check-no-aciu-on-production.sh` in the CI `unit` job per B12c).
...
Argo CD Image Updater (continuous, every 2m default poll)
  ...
  │           production-applicationset.yaml has ZERO ACIU annotations
  │           (enforced by scripts/ci/check-no-aciu-on-production.sh in CI).
  ...
  │    annotation needed: ACIU's default `image alias → images[].name`
...
- `infra/k8s/argocd/preview-applicationset.yaml` — `template.metadata.annotations` carries the two-alias ACIU block with distinct ImageNames: `app=ghcr.io/cogni-dao/cogni-template` (`app.allow-tags: regexp:^preview-[0-9a-f]{40}{{image_tag_suffix}}$`, `app.update-strategy: latest`) and `migrator=ghcr.io/cogni-dao/cogni-template-migrate` (`migrator.allow-tags: regexp:^preview-[0-9a-f]{40}{{migrator_tag_suffix}}$`, `migrator.update-strategy: latest`). Plus `write-back-method: git:secret:argocd/argocd-image-updater-git-creds`, `git-branch: main`, `write-back-target: kustomization`. **No** `migrator.kustomize.image-name` annotation — the GHCR split makes it redundant because the alias's ImageName (`cogni-template-migrate`) already matches the overlay's `images[].name` for the migrator entry.
- `infra/k8s/argocd/candidate-a-applicationset.yaml` — same annotation block as preview (Path A per B9), same `^preview-*` regex, same write-back target. `main`'s `candidate-a/{{name}}/kustomization.yaml` seed tracks the last known-good merged state; `candidate-flight.yml` continues to pin affected `pr-*` digests onto `deploy/candidate-a` at flight time via `promote-build-payload.sh` — ACIU and candidate-flight write to different branches and never race.
- `infra/catalog/{operator,poly,resy,scheduler-worker}.yaml` — `image_tag_suffix` + `migrator_tag_suffix` fields. `image_tag_suffix`: `""` / `"-poly"` / `"-resy"` / `"-scheduler-worker"`. `migrator_tag_suffix`: `"-operator-migrate"` / `"-poly-migrate"` / `"-resy-migrate"` / `"-scheduler-worker-migrate"` (the last is a no-op — scheduler-worker has no migrator; the `cogni-template-migrate` package has zero matching tags for it, ACIU's regex matches zero tags, and the alias silently skips). Both fields mirror `scripts/ci/lib/image-tags.sh:tag_suffix_for_target` and feed the AppSet template through the Git-file generator.
...
- [x] **B12 — Production path designed to 99% confidence.** User requirement: _"we need this working in candidate-a and preview (and 99% confident path for prod)."_ Design commitments, grounded in the real shape of `scripts/ci/promote-to-production.sh`:
  - (a) **Seed-freshness mechanism for production overlays on main — COMMITTED: option (iii) with an explicit rationale, NOT a fourth mirror workflow.** `promote-to-production.sh:102` reads digests from `deploy/preview` overlays directly into `deploy/production` via `extract_digests` + `promote-k8s-image.sh`. It does **not** read `main:infra/k8s/overlays/production/<app>/kustomization.yaml`. ACIU keeps `main:preview/` fresh → `rsync main → deploy/preview` on every preview flight keeps `deploy/preview` fresh → `promote-to-production.sh` reads `deploy/preview` → `deploy/production`. `main:production/` is not on the critical path for the live production environment; it is the base seed for a net-new overlay install (e.g. onboarding a new node). The "stale seed" class that ACIU exists to close on preview/candidate-a does not mechanically apply to production, because the promotion path doesn't read `main:production/` — it reads the already-fresh `deploy/preview`. Accepting drift on `main:production/` is correct, not lazy.

    The one narrow failure mode: if `promote-and-deploy.yml`'s `rsync` on push to `deploy/production` (the INFRA_K8S_MAIN_DERIVED invariant from bug.0334) ever re-seeds an untouched production service's overlay from a stale `main:production/` copy, an unrelated promotion could revert a currently-deployed service to an older digest. This is the concrete scenario that would promote option (ii) (post-ACIU mirror workflow) from "not needed" to "follow-up PR" — tracked by the staleness signal in (b). Not pre-emptively implemented, because pre-emptive mirroring creates the very class of main-based drift we're avoiding.
  ...
  - (b) **Staleness signal — COMMITTED recipe, implementation follow-up.** Grafana panel (on the existing Argo CD / deploy dashboard) with two Loki queries:
    ...
    # Query B: per-app digest-age on main's preview overlays (ACIU + flight-preview commits)
    count_over_time({job="github-actions", repo="cogni-template", branch="main"}
      | json | path=~"infra/k8s/overlays/preview/.*/kustomization.yaml" [30d])
    ```

    Alert rule: `production_commits_per_app == 0 over 30d AND preview_commits_per_app > 5`. If a production overlay for service X hasn't been touched in 30 days but preview has moved >5 digests, either (a) service X hasn't been promoted in a month (normal — ACIU-on-preview is doing its job and promote-to-production is the gate) or (b) `main:production/` is out of sync with reality and the next unrelated-flight rsync could revert a live service. The alert is advisory — it tells the operator to glance at `deploy/production`'s actual digest vs `main:production/`'s seed before the next promotion-to-production dispatch. Implementation tracked as a follow-up task under `proj.cicd-services-gitops` (not inside bug.0344, because it needs a separate dashboard JSON change and shouldn't gate bug.0344's merge).

  - (c) **Enforced rollback — SHIPPED in this PR.** `scripts/ci/check-no-aciu-on-production.sh` fails the CI `unit` job with a `::error file=infra/k8s/argocd/production-applicationset.yaml::` annotation if any `argocd-image-updater.argoproj.io/*` annotation is ever added to the production AppSet. Wired via `.github/workflows/ci.yaml` so every PR hits it. Paired with the bootstrap runbook's pre-flight check that `gh api repos/:owner/:repo/branches/main/protection | jq -e '.enforce_admins.enabled == false'` (drive-by note below) so the other half of the "ACIU silently 403s" failure mode is also surfaced at bootstrap time, not discovered in a controller log no one reads.
