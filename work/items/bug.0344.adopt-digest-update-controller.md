...
- [ ] **WRITE_BACK_SCOPE_EXCLUDES_NON_PREVIEW_OVERLAYS** — the image updater writes to `main` only, and only to `infra/k8s/overlays/preview/<app>/kustomization.yaml` — never to candidate-a overlays, never to production overlays, never to deploy branches. Deploy-branch digest promotion (`promote-k8s-image.sh` during flight workflows) stays as-is. (Candidate-a was descoped from auto-write post-#974 merge; see post-merge amendment below.) Verify: no `argocd-image-updater.argoproj.io/git-branch` annotation points at a `deploy/*` branch; the allowlist in `scripts/ci/check-image-updater-scope.sh` contains only `preview-applicationset.yaml`.
- [ ] **PRODUCTION_NOT_AUTO_UPDATED** — production flights require explicit human dispatch via `promote-to-production.yml`. The controller watches only the preview AppSet's Applications. Verify: the image-updater-scope check is green on every PR; `git grep 'argocd-image-updater' infra/k8s/argocd/production-applicationset.yaml` returns nothing.
...
Argo CD Image Updater (continuous, every 2m default poll)
  ├─ watches: Applications generated from preview-applicationset.yaml ONLY.
  │           candidate-a-applicationset.yaml was descoped post-#974
  │           merge (commit-rate vs. seed-freshness value — see post-merge
  │           amendment). production-applicationset.yaml is human-gated
  │           via promote-to-production.yml. Both exclusions are enforced
  │           by the positive allowlist in scripts/ci/check-image-updater-scope.sh
  │           (current allowlist: preview-applicationset.yaml only).
...
- `infra/k8s/argocd/candidate-a-applicationset.yaml` — **descoped post-#974 merge** (see post-merge amendment). Carries zero image-updater annotations; a docstring in the file records the rationale. `candidate-flight.yml` remains the sole writer of candidate-a overlay digests, via the rsync + `promote-k8s-image.sh` path.
...
- `scripts/ci/check-image-updater-scope.sh` + `.github/workflows/ci.yaml` (unit job) — B12(c) enforced invariant, widened post-#974: positive-allowlist gate that walks every `*-applicationset.yaml` under `infra/k8s/argocd/` and fails the `unit` job if any non-allowlisted file carries an `argocd-image-updater.argoproj.io` annotation. Current allowlist is `preview-applicationset.yaml` only — adding a new env requires editing the script's `ALLOWLIST` array with a committed rationale, not just a silent annotation. Production + candidate-a overlay digests are human- or flight-gated; this script is the structural backstop that stops a future well-intentioned edit from auto-committing on either env.
...
## Post-merge amendment (2026-04 — scope reduction, follow-up PR)

After PR #974 merged, a reproduction of the steady-state write rate surfaced an operational cost that had been under-weighted at design time: per merge-to-main, the image-updater fires one `Successfully updated image` commit per `(Application × alias)` pair whose live cluster digest has drifted from main's overlay seed. Across 4 preview Applications × 2 aliases (app + migrator) that's a theoretical upper bound of 8 commits/merge; observed steady-state is ~3–4, because scheduler-worker's migrator alias matches zero tags and not every service drifts on every merge. **For candidate-a the same write rate applies** — but with one critical difference from preview: `candidate-flight.yml` **unconditionally overwrites** every `deploy/candidate-a` overlay digest with the specific `pr-<N>-<sha>` tag under flight on every slot acquisition, rsyncing from `main → deploy/candidate-a` as the first step. This means **main's `infra/k8s/overlays/candidate-a/` seed is a developer-reference artifact only** — it is never read by the deploy path. The image-updater's writes to that seed therefore buy nothing operational; they only keep the reference fresh against the last merged `preview-*` tag class. Per-merge git noise vs. zero operational payoff was not a defensible trade.

**Scope changes shipped in the post-merge follow-up PR:**

1. `infra/k8s/argocd/candidate-a-applicationset.yaml` — the `template.metadata.annotations` block was stripped of all `argocd-image-updater.argoproj.io/*` entries. A docstring in the file records the rationale so a future `candidate-a` re-enablement is a deliberate design decision, not a silent re-add.
2. `scripts/ci/check-no-image-updater-on-production.sh` → `scripts/ci/check-image-updater-scope.sh` — renamed and reshaped from a two-file blocklist into a **positive allowlist** that walks every `*-applicationset.yaml` under `infra/k8s/argocd/`. Any non-allowlisted file carrying annotations fails CI. Allowlisted files missing annotations warn (not fail) so legitimate rollback PRs aren't blocked. Current allowlist: `preview-applicationset.yaml` only. Adding a new env requires editing `ALLOWLIST` in the script with a committed rationale.
3. `.github/workflows/ci.yaml` — `unit` job step renamed + re-pointed at `check-image-updater-scope.sh`.
4. `docs/runbooks/image-updater-bootstrap.md` — all three references to the old script name + narrative updated to reflect the preview-only scope and the allowlist mechanism.
5. This file — narrative above, invariants `WRITE_BACK_SCOPE_EXCLUDES_NON_PREVIEW_OVERLAYS` / `PRODUCTION_NOT_AUTO_UPDATED` updated, Wiring diagram updated, `### Modify` entry for `candidate-a-applicationset.yaml` flipped to "descoped", B12(c) rewritten to describe the allowlist.

**Invariants that survived unchanged (the carve-out is narrower, not different in kind):**

- `NO_BESPOKE_DIGEST_WORKFLOW`, `DIGEST_IMMUTABILITY_PRESERVED`, `COMMIT_PROVENANCE_VIA_MESSAGE_PREFIX`, `MAIN_WRITE_IS_NARROW_CARVE_OUT`, `APP_AND_MIGRATOR_BOTH_UPDATED`, `NO_NEW_GITHUB_APP`, `SIMPLE_SOLUTION`, `ARCHITECTURE_ALIGNMENT`, `ARGO_CD_IMAGE_UPDATER_BOOTSTRAP_IN_DEPLOY_INFRA` — none depend on candidate-a being in-scope. The `deploy-infra.sh` Step 7b controller reconcile continues to apply on all three environments (preview + candidate-a + production). The controller is installed everywhere, annotation-driven everywhere, and only does real work where annotations exist. A future re-enablement of candidate-a is a one-line annotation + allowlist edit, not an infrastructure change.

**Second post-merge finding — prettier vs. image-updater YAML-writer conflict:**

Within hours of #974 merging, the image-updater fired its expected cold-start burst against preview and committed 4 digest updates to `main:infra/k8s/overlays/preview/{operator,poly,resy,scheduler-worker}/kustomization.yaml`. The controller's Go YAML library produces output in a style that disagrees with prettier's YAML style on two dimensions: (a) list items emitted at column 0 vs. prettier's 2-space indent under parent keys, and (b) flow-style mappings as `{kind: Service, name: X}` vs. prettier's `{ kind: Service, name: X }`. Result: every PR merging to `main` after the image-updater fires was failing the `unit` job's `Format check` step (including PR #979 itself), because prettier sees those 4 files as "needs format-fix" while the image-updater will flatten them back on the very next merge. Prettier-fixing is not a fix — it's a one-cycle delay before the next machine write reverts it.

The structural fix shipped in this PR is `.prettierignore` excluding `infra/k8s/overlays/**/kustomization.yaml`. These files are machine-rewritten by two non-prettier authors (image-updater on `main:preview/`, `promote-k8s-image.sh` on deploy branches' overlays); prettier is simply not one of the authors, and gating CI on a format the machines won't preserve was never going to work. Humans who edit these by hand may still run prettier manually — we just don't block PRs on formatters that fight each other.
