...
`scripts/ci/wait-for-argocd.sh` issues a `hard-refresh + hook sync` kick every ~40s while the app is not-yet-synced. During the pull window, Argo is already in `phase=Running` waiting on the hook Job; the kick forces a `sync/terminate` of the in-flight op and starts a new syncId, which re-creates the Job with the same `BeforeHookCreation` policy — re-pulling the image, re-starting the 3m44s pull clock. The kick sometimes *extends* the wait, never shortens it (see the 9m14s run 24905074772 on 2026-04-24, where 4 concurrent syncIds stacked up in the controller log).
