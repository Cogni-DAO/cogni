...
- **(B) In-place Argo install** — land CP2a.1 (`infra/catalog/openbao.yaml` + AppSet generator) so Argo installs the substrate on the *existing* clusters, + a one-time OpenBao bootstrap job (init/unseal/seed/writer-role). **Preserves data**, less proven.
