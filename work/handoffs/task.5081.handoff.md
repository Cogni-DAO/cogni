...
- [x] **CP2a.0 — hub-runnable provisioner** (this PR, supersedes #1405): bring `provision-env-vm.sh` + lib dep-chain into the hub (byte-identical to node-template) and invert the fork guard so it runs on `Cogni-DAO/cogni`. Added alongside legacy `provision-test-vm.sh` — zero call-sites, zero runtime risk. Phase 5b (OpenBao init/unseal/auth-bind) confirmed present. **Needs Derek**: confirm B + a named same-day node-template porter (sync contract: backflow refactor) before merge.
