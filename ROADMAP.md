...
**Core nodes** are first-party nodes Cogni runs itself whose purpose is to _serve the rest of the network_, not just their own tenant — each a sovereign node (own repo, app, DB, knowledge hub, goal loop) but with cross-node syntropy as its job. The first is **`beacon`** — the network's growth engine: a closed loop of generate → broadcast (multi-channel: X, Moltbook, messaging) → attract → measure → attribute → refine that any node can call to market itself. `beacon` owns the **demand-side / acquisition analytics** (reach, engagement, traffic sources, funnel, marketing→signup→customer attribution); the operator keeps **supply-side / reliability analytics** (app performance, deploy health, infra). The **registry/librarian** follows (cross-node knowledge directory + search). Core nodes are not scopes inside the operator: they are separate repos, spawned via the node wizard, so their builder agents stay focused on their own domain and only touch their end of the node↔operator contract. What binds them into one product is shared at the seam, not the repo: core-node apps **cross-link in-product** (the operator's `/nodes` page → "Manage Beacon"), share **one model of _nodes_**, and resolve **one user identity across surfaces** (shared RBAC + DID interoperability) so a founder moves between node apps without re-auth. Node-owned goal loops run on operator-supplied **Temporal + LiteLLM** execution substrate — the node owns goal-setting and execution; the operator only provides the schedule plane and model gateway.
...
| **Core service nodes**             | First-party nodes that serve the whole network, not one tenant. First: `beacon` — the growth engine (generate → broadcast → attract → measure → attribute → refine) on the node-owned goal loop; owns demand-side acquisition analytics (operator keeps reliability analytics). Proves the shared-`nodes`-model + cross-node RBAC/DID-interop + in-product cross-linking pattern that every later core node reuses. | new node repo (spawned via wizard); goal loop → `node-template`                     |
...
**Status**: Operator-centered; reflects in-process consolidation (governance + review), the merge-authority convergence, and the first **core service node** (`beacon`, the growth engine) + shared-`nodes`-model / cross-node RBAC+DID direction
