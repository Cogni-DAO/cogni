...
| **Core service nodes**             | First-party nodes that serve the whole network, not one tenant. First: `creative` — AI-driven node marketing (generate → broadcast → analyze → improve) on the node-owned goal loop. Proves the shared-`nodes`-model + cross-node RBAC/DID-interop + in-product cross-linking pattern that every later core node reuses. | new node repo (spawned via wizard); goal loop → `node-template`                     |
...
**Status**: Operator-centered; reflects in-process consolidation (governance + review), the merge-authority convergence, and the first **core service node** (`creative`) + shared-`nodes`-model / cross-node RBAC+DID direction
