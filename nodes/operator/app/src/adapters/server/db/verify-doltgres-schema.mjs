...
/**
 * Module: `@operator-app/db/verify-doltgres-schema`
 * Purpose: Post-migration schema verification — read the latest drizzle-kit
 *   snapshot (the canonical "expected end state"), then introspect the live DB
 *   via information_schema and fail loud if any expected table/column/index is
 *   missing. Closes the silent-skip + silent-already-applied gaps in the
 *   Doltgres migration path.
 * Scope: Pure verification — no DDL, no tracking-table writes, no dolt_commit.
 *   Invoked by `migrate-doltgres.mjs` AFTER drizzle's `migrate()` and BEFORE
 *   any reconcileTracking / seed step.
 * Invariants:
 *   1. The newest snapshot (highest `idx` in `meta/_journal.json`) IS the
 *      authoritative expected end-state. Older snapshots are intermediate.
 *   2. Verification checks presence + nullability + primary-key flag, NOT
 *      arbitrary semantic equivalence. Type strings compared loosely
 *      (case-insensitive substring) — Doltgres's information_schema reports
 *      `character varying` for `text`, `timestamp with time zone` literally, etc.
 *   3. Failure throws synchronously — no silent stamping after this point.
 * Side-effects: IO (Postgres reads, filesystem reads).
 * Notes: Uses `sql.unsafe` because Doltgres 0.56's extended-protocol gap
 *   rejects parameterized queries — same pattern the migrator's recovery shim
 *   uses. The function only inspects what was generated by drizzle-kit;
 *   hand-edited DDL beyond drizzle's scope is out of scope (covered by the
 *   migrations-immutable CI guard).
 * Links: docs/spec/databases.md §2.6, nodes/operator/app/src/adapters/server/db/migrate-doltgres.mjs
 */
