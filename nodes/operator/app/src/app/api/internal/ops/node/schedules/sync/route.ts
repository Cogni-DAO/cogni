 ...
 * Purpose: Internal operations endpoint that triggers node-facing recurring-work schedule synchronization (story.5008).
 * Scope: Auth-protected POST endpoint for deploy automation. Delegates to the bootstrap job; does not implement sync logic or expose tenant-facing CRUD.
 * Invariants:
 *   - INTERNAL_OPS_AUTH: Requires Bearer INTERNAL_OPS_TOKEN (constant-time compare).
 *   - DISABLED_IS_NOOP: Returns 204 when NODE_SCHEDULES_ENABLED is false.
 *   - JOB_DELEGATION_ONLY: Uses runNodeSchedulesSyncJob() for all orchestration.
 * Side-effects: IO (HTTP request/response, DB advisory lock + Temporal RPC via the job).
 * Links: nodes/operator/app/src/bootstrap/jobs/syncNodeSchedules.job.ts, docs/design/node-temporal-tenant-interface.md
 * @internal
 */
