...
//
// Poly Doltgres migrator: drizzle-orm/postgres-js/migrator against
// `knowledge_poly`, plus a Doltgres-specific recovery path for the
...
//
// Why the recovery path: against `knowledge_poly` on Doltgres 0.56,
// drizzle-kit's `INSERT INTO drizzle.__drizzle_migrations VALUES ($1, $2)`
// raises XX000 ("table with name knowledge already exists" surfaces on the
// next restart) — the parameterized INSERT is rejected even though plain
// CREATE TABLE succeeds. The first poly migration (0000_init_knowledge)
// snuck through on an earlier Doltgres point release; subsequent migrations
// hit the gap. Same shim as operator's migrator (added in #1130) — both
// nodes must stay in sync. Removable when Doltgres closes the gap.
//
