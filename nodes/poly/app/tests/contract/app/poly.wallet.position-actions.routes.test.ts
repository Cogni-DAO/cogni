...
describe("poly wallet position action routes", () => {
  ...
  it("redeem route delegates to redeemResolvedPosition and maps expected executor failures", async () => {
    mockGetPolyTradeExecutorFor
      .mockResolvedValueOnce({
        redeemResolvedPosition: vi.fn().mockResolvedValue({
          tx_hash:
            "0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
        }),
      })
      .mockResolvedValueOnce({
        redeemResolvedPosition: vi
          .fn()
          .mockRejectedValue(
            new MockPolyTradeExecutorError("not_redeemable", "not redeemable")
          ),
      })
      .mockResolvedValueOnce({
        redeemResolvedPosition: vi
    ...
    expect(notRedeemable.status).toBe(409);
    await expect(notRedeemable.json()).resolves.toEqual({
