...
- \`core__poly_data_activity\` — lifecycle events (TRADE/SPLIT/MERGE/REDEEM/...). Realized-PnL reconstruction input.
- \`core__poly_data_traded_events\` — per-event counts per wallet. Category-focus signal.
...
2. For each candidate wallet: \`core__poly_data_value\` to drop sub-$1k wallets, then \`core__poly_data_positions\` + \`core__poly_data_activity\` to profile.
3. Use \`core__poly_data_traded_events\` to detect category specialization (sports, politics, crypto).
4. Filter by consistency — prefer ≥5 resolved markets, non-negative PnL across ≥3 distinct events, visible category focus.
5. If the user explicitly provides a market conditionId (hex 0x…), you MAY call \`core__poly_data_holders\` / \`core__poly_data_trades_market\` on it for market-specific counterparty harvest.
6. Return up to 5 candidates, ranked. Empty candidates are acceptable if nothing meets the bar.
