# lil-cog-doltgres-schema · AGENTS.md
...
- **Owners:** @derekg1729
- **Status:** stable
- **Package:** `@cogni/lil-cog-doltgres-schema`

## Purpose

Pure re-exports of the Drizzle table definitions from `@cogni/knowledge-base`, scoped to the lil-cog `knowledge_node_template` database. Owned by and namespaced to the lil-cog node. Mirrors `@cogni/operator-doltgres-schema` shape verbatim so every fork starts with the same versioned knowledge plane + work-items source-of-truth.
...
- **Subpath exports:**
  - `@cogni/lil-cog-doltgres-schema` — root barrel re-exports every slice
  - `@cogni/lil-cog-doltgres-schema/work-items` — `work_items` table + `WorkItemRow` / `NewWorkItemRow` inferred types
  - `@cogni/lil-cog-doltgres-schema/knowledge` — `knowledge_contributions` + `knowledge_contribution_commits` tables + syntropy seed re-exports from `@cogni/knowledge-base`

## Responsibilities

- **Does:** define Drizzle table schemas for lil-cog-local Doltgres tables.
- **Does not:** contain queries, adapters, business logic, RLS policies, or any I/O.

## Dialect separation (non-negotiable)

This package is globbed ONLY by `nodes/lil-cog/drizzle.doltgres.config.ts` (Doltgres target). `nodes/lil-cog/drizzle.config.ts` (Postgres target) MUST NOT include this path — if it did, the Postgres migrator would try creating Doltgres-only tables in lil-cog's Postgres DB.

## Migrator behavior (runs in lil-cog migrator initContainer)

```bash
# Container entrypoint for the Doltgres migration:
pnpm db:migrate:lil-cog:doltgres:container
```
