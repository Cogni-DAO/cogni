...
class FakeContributionPort implements KnowledgeContributionPort {
  ...
  private nextEdoContribution(
    principal: Principal
  ): { contributionId: string; branch: string } {
  ...
  async createEdoHypothesis(
    ...
    const { contributionId, branch } = this.nextEdoContribution(input.principal);
    const writes: Array<{ kind: "entry" | "citation"; id: string; branch: string }> = [
