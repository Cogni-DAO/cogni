...
- **Exports:**
  ...
  - `polyDataPositionsBoundTool`, `polyDataActivityBoundTool`, `polyDataValueBoundTool`, `polyDataHoldersBoundTool`, `polyDataTradesMarketBoundTool`, `polyDataTradedEventsBoundTool`, `polyDataResolveUsernameBoundTool`, `polyDataHelpBoundTool` + their `POLY_DATA_*_NAME` consts + `create*Implementation` factories — 8 Polymarket Data-API research tools (task.0368). All `read_only`. `help` is pure (no capability); other 7 consume `PolyDataCapability`.
  - `PolyDataCapability`, `PolyDataActivityType`, `PolyDataPositionsOutput`, `PolyDataActivityOutput`, `PolyDataValueOutput`, `PolyDataHoldersOutput`, `PolyDataMarketTradesOutput`, `PolyDataTradedEventsOutput`, `PolyDataResolveUsernameOutput` — shared capability + output types (runtime impl lives in `nodes/poly/app/src/bootstrap/capabilities/poly-research.ts`).
