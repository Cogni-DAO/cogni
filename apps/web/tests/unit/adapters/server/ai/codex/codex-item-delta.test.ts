...
describe("Codex agent_message delta extraction", () => {
  ...
  it("OLD BUG: global tracker would drop second message entirely", () => {
    // Demonstrate that a global tracker (never reset) drops the second message
    function extractDeltasOldBuggy(
      events: Array<{
        type: string;
        item?: { type: string; text: string };
      }>
    ): string[] {
      let fullText = ""; // <-- global, never reset
      const deltas: string[] = [];
    ...
    const events = [
      ...
      { type: "item.updated", item: { type: "agent_message", text: "First" } },
      {
        type: "item.completed",
        item: { type: "agent_message", text: "First" },
      },
      { type: "item.started", item: { type: "tool_call", text: "" } },
      { type: "item.completed", item: { type: "tool_call", text: "" } },
      // Second message — shorter than "First" so all deltas dropped
      { type: "item.started", item: { type: "agent_message", text: "" } },
      { type: "item.updated", item: { type: "agent_message", text: "Hi" } },
      { type: "item.completed", item: { type: "agent_message", text: "Hi" } },
    ];

    // Old buggy version drops the second message
    const buggyDeltas = extractDeltasOldBuggy(events);
    expect(buggyDeltas).toEqual(["First"]); // "Hi" is lost!

    // Fixed version captures both
    const fixedDeltas = extractDeltas(events);
    expect(fixedDeltas).toEqual(["First", "Hi"]);
  });
});
