...
| Granting Grafana/Loki **query** access (dev, validator, or a gate)   | [`docs/spec/grafana-observability-access.md`](../../../docs/spec/grafana-observability-access.md) — one admin root → 3 scoped consumers; **the operator API is NOT a Grafana proxy** (grant devs direct read-only `glsa_` tokens via RBAC). assertLive gates on the PUBLIC `run-carries` rung; the operator pod holds no query token.                                                              |
