...
**🔴 ROOT CAUSE FOUND + CURED (bug.5051, 2026-06-23) — it was a LEAK, not a need for auto-unseal.** The recurring reseals (preview ~3d, prod ~2.6d, candidate-a — all OOMKilled ~2026-06-20) were a deterministic **~9.3-day memory leak**, not the seal mechanism failing. Driver: the OpenBao HCL config carried `service_registration "kubernetes" {}`, but the `openbao` ServiceAccount has **no Role/RoleBinding** to get/patch its own pod → every registration attempt is **403 Forbidden, retried every 5s forever** (~17k/day), leaking memory from ~50Mi idle to the **512Mi** limit → OOMKill → Shamir reseal → silent multi-day secret-plane outage on every env. The registration is also **unused** (ESO targets the plain `openbao` Service; no `openbao-active`/`standby` Service exists, so the labels it sets route nothing). **Cure = delete the one dead config line** ([#1788](https://github.com/Cogni-DAO/cogni/pull/1788), merged + manually activated on candidate-a/preview/prod: 403=0, ~25–90Mi idle, 1Gi).

**Delivery gap that hid it (the deeper lesson).** #1788 (and #1617's 1Gi bump before it) edit `infra/k8s/argocd/openbao/values.yaml` — **shared-infra substrate that NO flight/promote pipeline delivers.** The 3 substrate Argo Applications (openbao/external-secrets/reloader) are applied **once at provision** (`register-substrate-apps.sh`), pinned to a deploy-branch ref, and never re-reconciled — so a merged substrate fix reaches a cluster only via a manual Argo-CR re-point (`targetRevision`→`main`, shipped in #1788) + a manual pod-roll (StatefulSet is `OnDelete`) + unseal. Same class as the **#1762 Reloader manual backfill**. Durable fix = an **app-of-apps** owning these 3 Application CRs. See [`cicd-platform-boundary.md`](../../../docs/spec/cicd-platform-boundary.md) § shared-infra substrate CD gap.

**On auto-unseal (this SUPERSEDES recorded-decision item 3 above).** The leak — not the seal mechanism — was the real problem, and it is cured. Hands-off auto-unseal is **not** the priority, and **cloud KMS is rejected**: this stack is Cherry bare-metal + OSS, "forkable to run fully sovereign" — a managed-cloud root-of-trust breaks sovereignty and would be our first cloud dependency ([#1791](https://github.com/Cogni-DAO/cogni/pull/1791), closed). The residual control for a _rare_ genuine reseal (reboot, chart bump) is the **sealed-state alert** (detect → quick manual unseal), 100% OSS. IF hands-off auto-unseal is ever truly required (real reseal pain post-cure, or a SOC2 deadline), the sovereign path is OpenBao-native `seal "transit"` against a self-hosted OpenBao — **never a cloud vendor.**
