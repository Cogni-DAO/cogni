...
| Granting Grafana/Loki **query** access (dev, validator, or a gate) | [`docs/spec/grafana-observability-access.md`](../../../docs/spec/grafana-observability-access.md) — one admin root → scoped consumers; operator is an **issuer, not a query proxy** (a grant-time issuance route is OK; a query-proxy route is rejected). v0 issues the **shared** env Viewer `glsa_` on `developer` grant (env-wide, NOT node-scoped); vNext needs a **label-scoped `glc_`** access-policy token (a Viewer `glsa_` CANNOT isolate by label) + a `node` Loki stream label. assertLive gates on the PUBLIC `run-carries` rung; the operator's gate holds no query token. Cross-substrate plane (Grafana/PostHog/DB/Temporal) + health scorecard: [`docs/spec/substrate-access-grant.md`](../../../docs/spec/substrate-access-grant.md). |
