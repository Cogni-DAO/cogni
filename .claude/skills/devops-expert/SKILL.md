...
| `resolve-pr-build-images.sh` | Resolve pushed PR image digests from GHCR for the `pr-{N}-{sha}` tag convention (used by candidate-flight, not the PR workflow). Includes per-node migrator tags (`{node}-migrator`) and the top-level `source_sha` envelope field consumed by `promote-build-payload.sh`.      |
| `promote-build-payload.sh`   | Apply resolved digests to a deploy-branch overlay via `promote-k8s-image.sh`. Tracks which apps actually got a new digest, emits `promoted_apps=<csv>` to `$GITHUB_OUTPUT`, and records per-app `source_sha` into `.promote-state/source-sha-by-app.json` (bug.0321 Fix 1 + 4). |
| `promote-k8s-image.sh`       | Lower-level overlay digest writer. Used directly by `promote-and-deploy.yml` and indirectly by `promote-build-payload.sh` / `promote-to-production.sh`. Treat as a primitive; never bypass it to edit overlay YAML by hand.                                                     |
...
| `wait-for-argocd.sh`              | SSH into the VM and block until every app in `PROMOTED_APPS` (CSV, or the full catalog when unset) reports `sync.revision == EXPECTED_SHA && Healthy`. `EXPECTED_SHA` MUST be the deploy-branch tip SHA (not the source-app commit). On success, writes `ARGOCD_SYNC_VERIFIED=true` to `$GITHUB_ENV` so downstream steps in the same job see the marker.                                                                                                                                                                               |
...
| `verify-buildsha.sh`              | End-of-deploy contract probe: curls `/readyz` on node-apps, asserts `.version` matches the expected SHA. Two modes: **per-app map** (`SOURCE_SHA_MAP=<path>` to `.promote-state/source-sha-by-app.json` — works for preview AND production since production overlays can mix SHAs cross-PR), and **single-SHA fallback** (`EXPECTED_BUILDSHA + NODES` — legacy, used when the map is missing on first deploy). Scheduler-worker and migrator are filtered out (no Ingress). Runs inside the `verify-deploy` / `verify-candidate` jobs. |
