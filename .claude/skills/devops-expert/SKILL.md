...
| `candidate-flight.yml`                   | workflow_dispatch                                                   | **App lever** for `candidate-a`: resolve PR digests from GHCR → write overlay → Argo reconciles pods. Three-job split (bug.0321): `flight` (promote + push) → `verify-candidate` (wait-for-argocd → readiness → verify-buildsha; gated on `promoted_apps != ''` at job level) → `release-slot`. No VM SSH for compose. (task.0314)                                                                                                                                                               |
