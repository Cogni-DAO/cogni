...
`PROMOTION_RUNS_AS_THE_OPERATOR` ([node-ci-cd-contract.md](../../../docs/spec/node-ci-cd-contract.md) § Env-promotion): for **child/submodule nodes**, every dispatch is an operator-GitHub-App action keyed off your registered Bearer token — drive the ladder over HTTP, don't `gh workflow run`.

> [!IMPORTANT]
> **The operator monorepo cannot self-flight via the API today (known shortcoming).** `POST /api/v1/vcs/flight {nodeRef}` is **child/submodule-node only** — it pins a submodule and dispatches `candidate-flight.yml` for that child. Passing the **operator's own** nodeId returns `409 invalid_catalog` ("invalid submodule node catalog entry for operator"). Flighting an **operator monorepo PR** to candidate-a is therefore still **100% `gh workflow run candidate-flight.yml -f pr_number=<n>`**, which requires `actions:write`/admin — an external agent (e.g. `flock-leader`) gets `403`, so a human must dispatch it. This is exactly what the **e2e operator-RBAC-controlled CICD** work is closing; until then, treat operator-self-flight dispatch as a human "initiate CI" step, then run `/validate-candidate` once candidate-a serves the SHA.

| Step                  | Call                                                                 | Authz (action → relation)                                                                      | Result                                                     |
| --------------------- | -------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| Flight a **child node** to candidate-a | `POST /api/v1/vcs/flight {nodeRef:{nodeId, sourceSha}}`              | `node.flight` → `can_flight` (`developer`)                                                     | `202` + `candidate-flight.yml` dispatch (candidate-a only) |
| Flight the **operator monorepo** to candidate-a | `gh workflow run candidate-flight.yml -f pr_number=<n>` (admin/`actions:write` — **no API path yet**) | GitHub repo admin (not RBAC) | `candidate-flight.yml` dispatch |
...
- **Child/submodule-node promote runs as the operator, never personal `gh`.** Flight/promote child nodes via the API (`vcs/flight`, `deploy/promote`); the operator GitHub App dispatches. `gh workflow run` of a child-node promote/flight uses a personal credential and is out of bounds. Read-only `gh run view/list` for watching a run is fine.
- **EXCEPTION — operator monorepo self-flight has NO API path yet (known shortcoming).** `vcs/flight {nodeRef}` is child-node-only (operator nodeId → `409 invalid_catalog`). Flighting an operator-monorepo PR is `gh workflow run candidate-flight.yml -f pr_number=<n>`, which needs admin/`actions:write` — so it's a human "initiate CI" step, not an external-agent self-serve action. The e2e operator-RBAC-controlled CICD work closes this.
